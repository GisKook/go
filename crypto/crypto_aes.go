...
func CryptoAesCBCEncrypt(_plaintext string, key []byte) ([]byte, error) {
	plaintext := padding(_plaintext)
...
func CryptoAesCBCDecrypt(ciphertext []byte, key []byte) ([]byte, error) {
	block, err := aes.NewCipher(key)
	if err != nil {
		return nil, err
	}
